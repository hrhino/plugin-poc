...
abstract class SufficiencyChecker
  ...
  val MinimalAttr = symbolOf[minimal]
  ...
  object error {
    ...
    def MissingImplementations(sym: Symbol, formula: solver.Formula): Unit =
      globalError(sym.pos,
        s"missing implementation for methods: ${solver.showFormula(formula)}"
      )
    def MinimalAnnotationOnNonTrait(sym: Symbol, pos: Position): Unit =
      globalError(pos,
        s"@${MinimalAttr.name} annotations may only be applied to traits")
